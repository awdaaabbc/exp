# Model3 Sensitivity BCI 多数据集训练框架

本项目是基于 Model3_Sensitivity 模型的多数据集训练框架，支持 BCI IV 2a、BCI IV 2b 和 OpenBMI 三个数据集的训练和敏感度分析。
...
│   ├── models/                      # 模型定义
│   │   └── model3_sensitivity.py    # Model3 敏感度模型
│   └── training/                    # 训练相关
│       └── trainer.py               # 训练器基类
│
├── checkpoints/                     # 模型checkpoint保存目录
│   └── best_model3_sensitivity/     # 各受试者最佳模型
│
├── data/                            # 数据目录（本地挂载点，不提交到git）
...
| 文件 | 说明 |
|------|------|
| `train_model3_sensitivity.py` | BCI IV 2a 敏感度分析训练入口，支持多专家频率分割实验 |
...
| 文件 | 说明 |
|------|------|
| `bci_iv_2a_model3_sensitivity.yaml` | BCI 2a 模型和数据配置 |
...
| `model3_sensitivity.py` | Model3 敏感度模型定义，包含 DynamicTemporalConv 等组件 |
...
- **路径配置**：修改 `configs/bci_iv_2a_model3_sensitivity.yaml` 中的 `data_path`
...
#### BCI 2a
修改 `configs/bci_iv_2a_model3_sensitivity.yaml`：
```yaml
...
```powershell
python train_model3_sensitivity.py
```

可选环境变量：
```powershell
$env:BCI_DATA_PATH="D:\你的路径\bci_iv_2a1"
$env:EPOCHS_OVERRIDE="1"       # 覆盖配置文件中的epoch数
